...
#'
#' @inheritParams read_ukb
#' @param data_dict_colname_col character. The column in \code{ukb_data_dict} with
...
tidy_clinical_events_basis <- function(ukb_main,
  ...
  assertthat::assert_that(
    ...
    all(code_col_field_id %in% ukb_data_dict$FieldID) &
      (date_col_field_id %in% ukb_data_dict$FieldID),
    msg = paste0(
      "Error! Both FieldID ",
      stringr::str_c(code_col_field_id, sep = "", collapse = ", "),
      " and FieldID ",
      date_col_field_id,
      " are required, one or both of these is not present in ukb_data_dict"
    )
  )
