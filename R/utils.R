...
#' by an environmental variable named `UKB_DICT` (see
...
#' )
get_ukb_dict <- function(path = NULL) {
  get_ukb_dict_codings_helper(
    path = path,
    env_var_name = "ukb_dict",
...
#' Helper for get_ukb_dict() and get_ukb_codings()
#'
#'
#'
#' @return Data frame.
#' @noRd
get_ukb_dict_codings_helper <- function(path,
                                             env_var_name,
                                             file_name,
                                             download_url) {
  if (is.null(path)) {
...
#' @param ukb_data_dict A data dictionary generated by \code{\link{make_data_dict}}
...
#' @param colname_col The column in \code{ukb_data_dict} containing the column names
...
#'
#' @param ukb_data_dict a data dictionary generated by \code{\link{make_data_dict}}
#' @param filter_col character. The column in \code{ukb_data_dict} to filter on.
#' @param filter_value character (single or multiple). Filter for values in
#'   \code{filter_col} that match one of these.
#' @param return_col character. The column in \code{ukb_data_dict} to return after
#'   filtering.
...
filter_data_dict <- function(ukb_data_dict,
                             filter_col,
                             filter_value,
                             return_col,
                             error_if_missing = TRUE) {
  # filter
  result <- ukb_data_dict %>%
    dplyr::filter(.data[[filter_col]] %in% filter_value) %>%
    .[[return_col]]

  # missing filter_values
  missing_filter_values <- subset(filter_value, !filter_value %in% ukb_data_dict[[filter_col]])
  ...
  } else if (any(!filter_value %in% ukb_data_dict[[filter_col]]) &
    (error_if_missing == TRUE)) {
    stop(
      paste0(
