...
#'   those under the \code{descriptive_colnames} column in \code{ukb_data_dict}.
...
summarise_numerical_variables <- function(ukb_main,
  ...
  if (is.null(ukb_data_dict)) {
    ukb_data_dict <- make_data_dict(ukb_main,
      ukb_dict = ukb_dict
    )
  } else if (!is.null(ukb_data_dict)) {
    assertthat::assert_that(all(ukb_data_dict$colheaders_raw %in% names(ukb_main)),
      msg = "Error! `ukb_data_dict` does not match `ukb_main`. All values in `colheaders_raw` should be present in `names(ukb_main)`. Try making a new data dictionary with `make_data_dict()`?"
    )
  }
  ...
  numerical_vars_to_summarise <- ukb_data_dict %>%
