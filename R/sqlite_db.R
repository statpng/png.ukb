...
make_clinical_events_db <- function(ukb_main_path,
  ...
  table_already_present_in_db <- subset(
    tables_to_write,
  ...
  ukb_data_dict <- make_data_dict(ukb_main_path,
    delim = ukb_main_delim,
    ukb_dict = ukb_dict
  )

  # check that eid col is present
  assertthat::assert_that("eid" %in% ukb_data_dict$FieldID,
  ...
  missing_clinical_events_fields <-
    subset(available_clinical_events_fields, !(available_clinical_events_fields %in% ukb_data_dict$FieldID))
  ...
  # filter `ukb_data_dict` for 'eid' and clinical events fields only
