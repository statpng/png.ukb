...
#'
#' Creates a data dictionary for a raw UK Biobank main dataset, either using
#' file path or from a data frame if the dataset has already been loaded into R.
#'
...
#'   [data.table::fread()]). Ignored if the file name ends with \code{.dta}
#'   (i.e. is a STATA file) or if \code{ukb_main} is a data frame.
#' @param ukb_dict The UKB data dictionary (available online at the UK
#'   Biobank
...
make_data_dict <- function(ukb_main,
  ...
  ukb_data_dict <- ukb_data_dict %>%
    dplyr::mutate(
      "unrecognised_fid" = dplyr::case_when(
        !(.data[["FieldID"]] %in% c("eid", ukb_dict$FieldID)) ~ TRUE,
        TRUE ~ FALSE
      )
    ) %>%
  ...
  ukb_data_dict <- mutate_descriptive_columns(ukb_data_dict = ukb_data_dict)

  # add ValueType column for eid - type 'Integer'
  ukb_data_dict$ValueType <- ifelse(ukb_data_dict$FieldID == "eid",
    yes = "Integer",
    no = ukb_data_dict$ValueType
  )

  # update 'Field' to 'Participant identifier' for 'eid'
  ukb_data_dict$Field <- ifelse(ukb_data_dict$FieldID == "eid",
...
#' @param ukb_data_dict A data dictionary specific to the UKB main dataset file,
...
#' read_ukb(
#'   path = dummy_ukb_main_path,
#'   ukb_data_dict = data_dict_selected,
#'   ukb_dict = dummy_ukb_dict,
#'   ukb_codings = dummy_ukb_codings
#' )
#'
...
read_ukb <- function(path,
  ...
  if( !is.null(ukb_data_dict) ){
    argument.dict <- deparse(substitute(ukb_data_dict))
  } else {
    argument.dict <- deparse(substitute(ukb_dict))
  }
  ...
  # make ukb_data_dict if not supplied
  if (is.null(ukb_data_dict)) {
    message("Creating data dictionary")
    ukb_data_dict <- make_data_dict(
      ukb_main = path,
      delim = delim,
      ukb_dict = ukb_dict
    )
  ...
  if (descriptive_colnames) {
    ...
    result <- rename_cols(
      df = result,
      old_colnames = ukb_data_dict[["colheaders_raw"]],
      new_colnames = ukb_data_dict[["descriptive_colnames"]]
    )
  }
  ...
  attr(result, "argument") <- argument.dict
...
#' @param colnames_col The name of the column in \code{ukb_data_dict} that contains
...
label_ukb_main <- function(ukb_main,
  ...
  # check that ukb_main and ukb_data_dict match
  assertthat::assert_that(all(ukb_data_dict[[colnames_col]] %in% names(ukb_main)),
    msg = "There are values in `ukb_data_dict[[colnames_col]]` that do not match `names(ukb_main)`. Check `ukb_data_dict`."
  )

  # warning if any fields in `ukb_data_dict` do not have a FieldID
  data_dict_fields_with_no_fid <- ukb_data_dict[is.na(ukb_data_dict$FieldID), ]$FieldID
  if (!rlang::is_empty(data_dict_fields_with_no_fid)) {
    warning(paste0(
      "The following items in `ukb_data_dict` could not be matched to a FieldID: ",
      stringr::str_c(data_dict_fields_with_no_fid,
        sep = "",
        collapse = ", "
      )
    ))
    # remove these items from ukb_data_dict
    ukb_data_dict <- ukb_data_dict[!is.na(ukb_data_dict$FieldID), ]
  }

  # identify codings to be converted to integer type
  ukb_data_dict <- indicate_coltype_in_data_dict(
    ukb_data_dict = ukb_data_dict,
    ukb_codings = ukb_codings
  )
  ...
  ukb_codings <- ukb_codings %>%
    dplyr::filter(.data[["Coding"]] %in% ukb_data_dict[["Coding"]])
  ...
  # set 'Coding' col in `ukb_data_dict` to 'NA' if excluded from `ukb_codings`
  ukb_data_dict$Coding <- ifelse(ukb_data_dict$Coding %in% ukb_codings$Coding,
    yes = ukb_data_dict$Coding,
    no = NA
  )

  # check that column types match expected coltype from ukb_data_dict
  expected_coltypes <- as.list(stats::setNames(
    ukb_data_dict$col_types_fread,
    ukb_data_dict[[colnames_col]]
  ))
  ...
  assertthat::assert_that(
    ...
    msg = paste0(
      ...
      ". Try `ukbwranglr:::indicate_coltype_in_data_dict(ukb_data_dict = ukb_data_dict, ukb_codings = ukb_codings)` and check the values under column `col_types_fread`"
    )
  )

  # create column labels
  ukb_data_dict[["variable_label"]] <- ifelse(
    ukb_data_dict[[colnames_col]] == "eid",
    yes = "eid",
    no = paste0(
      ukb_data_dict$Field,
      " (",
...
mutate_descriptive_columns <- function(ukb_data_dict) {
  ...
  ukb_data_dict[["descriptive_colnames"]] <- column_names
  ukb_data_dict[["Field_FieldID"]] <- Field_FieldID_names
  ...
  duplicated_descriptive_colnames <- ukb_data_dict %>%
  ...
  if (nrow(duplicated_descriptive_colnames) > 0) {
    assertthat::assert_that(length(unique(ukb_data_dict$colheaders_raw)) == nrow(ukb_data_dict),
  ...
  ukb_data_dict$descriptive_colnames <- stringr::str_remove(ukb_data_dict$descriptive_colnames,
    pattern = "[_na]+$"
  )
  ...
  ukb_data_dict$descriptive_colnames <- ifelse((stringr::str_detect(
    string = ukb_data_dict$colheaders_raw,
    pattern = ukb_data_dict$Field_FieldID
  )) &
    (ukb_data_dict$Field_FieldID != "eid"),
  yes = ukb_data_dict$colheaders_raw,
  no = ukb_data_dict$descriptive_colnames
  )

  # make `descriptive_colnames` = `colheaders_raw` if fields/instance/array are
  # all `NA`
  ukb_data_dict$descriptive_colnames <- ifelse((is.na(ukb_data_dict$FieldID)) &
    (is.na(ukb_data_dict$instance) &
      is.na(ukb_data_dict$array)),
  yes = ukb_data_dict$colheaders_raw,
  no = ukb_data_dict$descriptive_colnames
  )

  # remove Field_FieldID column
  ukb_data_dict$Field_FieldID <- NULL

  return(ukb_data_dict)
}

indicate_coltype_in_data_dict <- function(ukb_data_dict,
                                          ukb_codings) {
  # helper for `read_ukb*`
...
read_ukb_raw_basis <- function(path,
  ...
  coltypes_fread <- stats::setNames(
    ukb_data_dict$col_types_fread,
    ukb_data_dict$colheaders_raw
  )

  coltypes_readr <- stats::setNames(
    ukb_data_dict$col_types_readr,
    ukb_data_dict$colheaders_raw
  ) %>%
    as.list() %>%
  ...
  switch(read_with,
    ...
    read_dta = haven::read_dta(
      file = path,
      col_select = tidyselect::all_of(ukb_data_dict$colheaders_raw),
      n_max = nrows,
      ...
    ) %>%
...
#'
#' @param df data frame
#' @param ukb_data_dict data dictionary
...
read_ukb_chunked_to_file <- function(in_path,
  ...
  # check that ukb_main and ukb_data_dict match
  assertthat::assert_that(all(ukb_data_dict$colheaders_raw %in% data_dict_full$colheaders_raw),
    msg = "Values in `ukb_data_dict$colheaders_raw` do not match column names for file at `in_path`. Check `ukb_data_dict`."
  )
    ...
    function(x, pos) {
      ...
      if (descriptive_colnames) {
        ...
        x <- rename_cols(
          df = x,
          old_colnames = ukb_data_dict[["colheaders_raw"]],
          new_colnames = ukb_data_dict[[NEW_COLNAMES_COL]]
        )
